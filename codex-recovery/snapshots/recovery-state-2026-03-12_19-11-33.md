...
## Operator Note
Verified current unused move wave and reran build; locked next block to workstation media path/link remediation with medium issues deferred.
...
- Keep the next execution block limited to the catalog/media remediation lane only.
- Start with the verified workstation set: `curvivo`, `deskpro`, `sleek`, `panel-pro`, and `x-bench`.
- Treat `curvivo`, `deskpro`, `sleek`, and `panel-pro` as path-correction/data-link fixes because canonical asset folders and `image-1.webp` plus `image-2.webp` already exist under `public/images/catalog/oando-workstations--*`.
- Treat `x-bench` as a catalog-link gap, not an asset-sourcing problem, because `public/images/catalog/oando-workstations--x-bench/` already exists with `image-1.webp` and `image-2.webp`.
- Keep medium metadata issues deferred until the critical/high workstation media mapping is repaired or explicitly blocked.
- Keep `oando_website/*`, held duplicate families, remaining root tool/artifact cleanup, and uncertain routes out of this work block.
