...
- Timestamp: 2026-03-12T22:56:44
- Branch: main
- Working tree entries: 232
- Modified: 48
- Added: 0
- Deleted: 149
- Renamed: 0
...
## Operator Note
Finalized resumed block docs and sync verification hardening; single next step is legacy runtime dependency review.
...
- [x] Record that the previous handover is superseded because active code work resumed
- [x] Record the exact live Supabase baseline: `products=145`
- [x] Record the exact live Supabase baseline: `business_stats_current=1`
- [x] Record that Nhost GraphQL live product read is verified
- [x] Record that Nhost GraphQL live business-stats read is verified
- [x] Record the exact live Nhost SQL baseline: `products=145`
- [x] Record the exact live Nhost SQL baseline: `business_stats_current=1`
- [x] Record that canonical product parity currently matches between Supabase and Nhost SQL counts
- [x] Lock the internal fallback-source contract to `supabase | nhost-graphql | nhost-sql | stale-cache | safe-default`
- [x] Record that business-stats fallback no longer uses a collapsed `nhost-backup` source label
- [x] Confirm no app or test surface still depends on the old `nhost-backup` literal
- [x] Update `DECISIONS.md` with the baseline and source-contract lock
- [x] Update `RECOVERY-CHECKLIST.md` to mark the first 24-hour platform items complete

### Middle 24 Hours

- [x] Confirm `lib/types/businessStats.ts` reflects split Nhost source labels
- [x] Confirm `lib/nhostBackup.ts` uses GraphQL first
- [x] Confirm `lib/nhostBackup.ts` falls back to SQL when GraphQL is missing, denied, schema-broken, times out, or returns no active stats row
- [x] Confirm `lib/businessStats.ts` uses `Supabase -> Nhost GraphQL -> Nhost SQL -> stale cache -> safe default`
- [x] Confirm stale cache is only used after both Nhost fallback paths fail
- [x] Confirm safe defaults are only used after Supabase, Nhost GraphQL, Nhost SQL, and stale cache all fail
- [x] Confirm `tests/stats-consistency.spec.ts` accepts `nhost-graphql` and `nhost-sql`
- [x] Re-run build verification after the business-stats fallback changes
- [x] Record the build result in recovery docs
- [x] Mark Phase 21 source-label work complete in `RECOVERY-CHECKLIST.md`
- [x] Mark Phase 21 SQL-rescue work complete in `RECOVERY-CHECKLIST.md`
- [x] Mark Phase 21 fallback-order alignment complete in `RECOVERY-CHECKLIST.md`

### Final 24 Hours

- [x] Expand `scripts/sync_nhost_backup.ts` verification scope from minimal connectivity to full mirror-contract verification
- [x] Add canonical row-count verification for `catalog_products`
- [x] Add canonical row-count verification for `catalog_categories`
- [x] Add canonical row-count verification for `catalog_product_specs`
- [x] Add canonical row-count verification for `catalog_product_images`
- [x] Add canonical row-count verification for `catalog_product_slug_aliases`
- [x] Add row-count verification for `business_stats_current`
- [x] Add legacy row-count verification for `products`
- [x] Add legacy row-count verification for `categories`
- [x] Add legacy row-count verification for `product_specs`
- [x] Add legacy row-count verification for `product_images`
- [x] Add legacy row-count verification for `product_slug_aliases`
- [x] Add GraphQL verification for reading one mirrored product by slug
- [x] Add GraphQL verification for reading active business stats
- [x] Add GraphQL verification for alias lookup when alias rows exist
- [x] Add SQL verification for reading one mirrored product by slug
- [x] Add SQL verification for reading active business stats
- [x] Add SQL verification for alias lookup when alias rows exist
- [x] Fail sync on any count mismatch
- [x] Fail sync on any missing GraphQL verification read
- [x] Fail sync on any missing SQL verification read
- [x] Record the remaining legacy runtime dependency review after verification expansion
- [x] Update `NEXT-PLAN.md`, `DECISIONS.md`, and `RECOVERY-CHECKLIST.md` to reflect what remains
- [x] Generate a fresh recovery snapshot
- [x] Update `latest.md`
- [x] Leave the next explicit step as either `legacy dependency review` or `Week 2 catalog recovery`, depending on completion state
...
- [x] Live parity baseline is recorded with exact counts.
- [x] Internal fallback-source contract is locked and reflected in docs.
- [x] Business-stats fallback code and tests match the target precedence.
- [x] Build verification passes after fallback changes.
- [x] Mirror verification requirements are explicit enough to implement without guessing.
- [x] Recovery docs reflect resumed work instead of the stale handover state.
- [x] The 72-hour platform follow-on is explicit across first, middle, and final 24-hour task slices.
...
- Complete legacy runtime dependency review for any remaining reads tied to legacy Nhost tables, then decide whether Week 1 can close into Week 2 catalog recovery.
