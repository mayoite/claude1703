...
## Operator Note
Added an explicit HOLDS.md resolution pass to the recovery plan and queued it after the current platform follow-on.
...
Carry the now-locked basics-first lane into the platform contract follow-on without reopening image or hardcoding scope.
...
- [ ] Revisit `codex-recovery/HOLDS.md` immediately after the current platform-follow-on block closes
- [ ] Classify each hold as `keep protected`, `resolve ownership`, `archive candidate`, or `separate dirty-state investigation`
- [ ] Resolve the highest-signal hold groups first: duplicate component families, protected data/config surfaces, dynamic-import surfaces, then pre-existing dirty-state holds
...
- [ ] A hold-resolution pass is explicitly queued after the current platform follow-on.
