...
## Operator Note
Recorded exact catalog and business-stats fallback touchpoints, normalized the Supabase/Nhost mirror contract, and converted the architecture lane into an implementation queue.
...
- Keep the next execution block limited to the Supabase/Nhost architecture follow-up lane only.
- Lock the recorded current runtime touchpoints around `lib/getProducts.ts`, `lib/nhostCatalog.ts`, `lib/businessStats.ts`, `lib/nhostBackup.ts`, and `scripts/sync_nhost_backup.ts`.
- Treat Supabase as the only write authority; treat Nhost as a read-only mirror for runtime continuity.
- Keep Nhost dual-table compatibility during the transition by mirroring both `catalog_*` tables and the legacy table names.
- Keep long-term dual-path runtime fallback explicit: catalog reads use Nhost GraphQL first with SQL rescue, and business-stats fallback becomes Supabase -> Nhost GraphQL -> Nhost SQL -> stale cache -> safe default.
- Implement the next decision-ready architecture tasks in this order: source labeling, business-stats SQL rescue, fallback precedence alignment, mirror verification expansion, then legacy-dependency review.
- Record explicit failure conditions, source labeling, and verification expectations for both GraphQL and SQL mirror reads in recovery docs as code changes land.
- Keep workstation media work, duplicate-family cleanup, root artifact cleanup, and `oando_website/*` investigation out of this work block.
- Use `codex-recovery/OPEN-QUESTIONS.md` only to capture non-blocking questions; do not pause for them unless a real blocker appears.
