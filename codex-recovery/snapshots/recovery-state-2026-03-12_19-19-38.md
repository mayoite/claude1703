...
## Operator Note
Promoted Supabase/Nhost architecture follow-up to the active next block and added a non-blocking questions file so work can proceed without pausing.
...
- Lock the current runtime touchpoints around `lib/getProducts.ts`, `lib/nhostCatalog.ts`, `lib/businessStats.ts`, `lib/nhostBackup.ts`, and `scripts/sync_nhost_backup.ts`.
...
- Record explicit failure conditions, source labeling, and verification expectations for both GraphQL and SQL mirror reads.
