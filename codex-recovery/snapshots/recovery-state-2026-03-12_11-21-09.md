...
## Operator Note
45-minute recovery cycle enabled; next plan now embedded in every snapshot
...
- Verify the moved proven-dead files now exist under `unused/` with preserved relative paths.
- Re-run `git status --short` and record the structural move wave separately from older dirty state.
- Run `npm run build` after the move wave and capture whether any import or path breakage remains.
- If build passes, update `codex-recovery/RECOVERY-CHECKLIST.md`, `codex-recovery/DECISIONS.md`, and `codex-recovery/HOLDS.md` with the verified result.
