...
## Operator Note
Merged the live HOLDS.md queue into NEXT-PLAN.md while keeping HOLDS.md as the backing reference.
...
# Week 1 / Fix Basics First / Current 45-Minute Block

Execution rule for this block: fix basics first before wider platform-overhaul follow-ons.

## Objective

Carry the now-locked basics-first lane into the platform contract follow-on, with the live hold queue merged into this plan instead of treated as a separate side file.
...
- [x] Re-enter the Supabase/Nhost contract lane only after the basics-first lock is recorded
- [ ] Keep the next platform tasks ordered as parity baseline, fallback-source contract, business-stats SQL rescue design, and mirror verification expansion

### Hold Resolution Follow-On

- [ ] Keep `codex-recovery/HOLDS.md` as the backing record, but resolve the live hold queue from this plan
- [ ] Classify each hold as `keep protected`, `resolve ownership`, `archive candidate`, or `separate dirty-state investigation`
- [ ] Record hold-resolution outcomes back into recovery docs instead of letting `HOLDS.md` remain a static parking area

### Merged Hold Queue

#### Preview/Internal And Route-Owned Holds

- [ ] Keep `app/home-unused/page.tsx` protected unless a later route/product decision reclassifies it
- [ ] Keep `app/ops/customer-queries/page.tsx` protected as an internal operations route unless explicitly replaced
- [ ] Keep everything under `app/` out of ordinary dead-file cleanup until route ownership is intentionally reviewed

#### Duplicate-Family Holds

- [ ] Resolve `components/layout/*` as `legacy keep` or `archive/remove candidate`
- [ ] Keep `components/site/*` protected as the active shell owner
- [ ] Resolve the legacy configurator family: `components/configurator/Configurator.tsx`, `components/configurator/configurator/Configurator.tsx`, `components/configurator/ConfiguratorContext.tsx`, `components/configurator/ConfiguratorPreview.tsx`, and `components/configurator/ConfiguratorSteps.tsx`
- [ ] Resolve `components/shared/ProcessSection.tsx` against the live owner `components/home/ProcessSection.tsx`
- [ ] Resolve `components/ui/CookieConsent.tsx` against the live owner `components/site/CookieConsentBar.tsx`
- [ ] Resolve `components/bot/AdvancedBot.tsx` against the dynamic bot path that uses `DynamicBotWrapper` and `UnifiedAssistant`

#### Protected Data And Config Holds

- [ ] Resolve `data/site/configurator.ts` as `active config` or `hold for later configurator review`
- [ ] Resolve `data/site/support.ts` as `active config` or `hold for later support-surface review`

#### Archived And Preserved Holds

- [ ] Keep `archive/duplicate-assets/root-assets/*` as preserved archive, not active cleanup target
- [ ] Keep `docs/ops/artifacts/*`, `docs/ops/charts/*`, and `docs/ops/plans/*` as preserved reference areas, not active source cleanup targets
- [ ] Keep `unused/` protected as the preserved dead-code boundary and do not let it become a dump area for uncertain files

#### Dynamic-Import Holds

- [ ] Keep `app/layout.tsx`, `components/bot/DynamicBotWrapper.tsx`, and `components/bot/UnifiedAssistant.tsx` protected from static zero-import cleanup unless dynamic usage is intentionally retired

#### Pre-Existing Dirty-State Hold

- [ ] Keep `oando_website/*` classified as separate pre-existing dirty state until a dedicated investigation block reclassifies it
...
- [ ] The next platform-follow-on step is still explicit after the basics-first block closes.
- [ ] The merged hold queue is explicit enough to resolve from `NEXT-PLAN.md` without opening a separate planning lane.
