...
## Operator Note
Resolved all live hold entries into explicit outcomes; HOLDS.md is now empty and hold outcomes live in decisions/checklist.
...
- [x] Keep `codex-recovery/HOLDS.md` as the backing record for unresolved items only
- [x] Resolve the live unresolved hold groups after the current platform-follow-on block: preview/internal reclassification, duplicate-family holds, protected data/config holds, and `oando_website/*`
- [x] Classify each resolved hold as `keep protected`, `resolve ownership`, `archive candidate`, or `separate dirty-state investigation`
- [x] Record hold-resolution outcomes back into recovery docs and remove resolved entries from `HOLDS.md`
...
- [x] The hold follow-on is explicit enough to resolve live unresolved entries from `HOLDS.md` without duplicating settled rules in multiple files.
