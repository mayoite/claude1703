...
## Operator Note
Cleaned HOLDS.md to unresolved-only entries and trimmed NEXT-PLAN hold content to active queued hold work only.
...
- [ ] Keep `codex-recovery/HOLDS.md` as the backing record for unresolved items only
- [ ] Resolve the live unresolved hold groups after the current platform-follow-on block: preview/internal reclassification, duplicate-family holds, protected data/config holds, and `oando_website/*`
- [ ] Classify each resolved hold as `keep protected`, `resolve ownership`, `archive candidate`, or `separate dirty-state investigation`
- [ ] Record hold-resolution outcomes back into recovery docs and remove resolved entries from `HOLDS.md`
...
- [ ] The hold follow-on is explicit enough to resolve live unresolved entries from `HOLDS.md` without duplicating settled rules in multiple files.
