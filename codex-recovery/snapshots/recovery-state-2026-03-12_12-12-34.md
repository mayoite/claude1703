...
## Operator Note
Completed explicit root inventory classification, confirmed dynamic-import holds around the bot wrapper chain, and separated the 42-file unused move wave from the older 52-path oando_website deletion cluster.
...
- Start Phase 13 route truth by inventorying the routes under `app/` and classifying them as public, internal, preview, redirect/alias, or uncertain.
- Preserve the current duplicate-family holds while route truth is being mapped.
- Avoid any new source moves until route truth notes exist.
