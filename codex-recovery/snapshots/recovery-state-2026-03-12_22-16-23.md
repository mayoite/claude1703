...
## Operator Note
Expanded NEXT-PLAN into a checklist-driven block and recorded the rule that future next plans should always include a checklist.
...
Turn the basics-first lane into an execution-ready checklist so missing-image repair and hardcoding cleanup are locked before the platform contract follow-on resumes.

## Immediate Sequence

1. Lock the missing-image repair lane from the current audit baseline.
2. Lock the hardcoding cleanup lane from the current overhaul notes.
3. Define validation for both basics-first lanes.
4. Continue the Supabase/Nhost platform contract work after the basics-first lock.
...
- [ ] Confirm the critical/high issue source stays `docs/product-quality-audit.csv`
- [ ] Keep the first repair set explicit: `curvivo`, `deskpro`, `sleek`, `panel-pro`, `x-bench`
- [ ] Separate invalid-path fixes from true missing-coverage fixes
- [ ] Record what is ready for path correction versus what still needs catalog-link repair

### Hardcoding Lane

- [ ] Confirm the initial hardcoding source stays `docs/tasks/full-overhaul-plan.md`
- [ ] Keep scope limited to metadata, JSON-LD, footer/nav labels, route-copy literals, assistant prompt/content arrays acting as content storage, category/route mapping literals, and fallback/public-facing copy that should be centralized
- [ ] Keep design tokens, intentional constants, and low-risk one-off UI text out of this first hardcoding pass unless already classified in the overhaul notes
- [ ] Record the first extraction/cleanup targets clearly enough that implementation can start without re-deciding scope

### Validation Lock

- [ ] Missing-image validation is explicit for primary image paths and minimum gallery coverage
- [ ] Hardcoding validation is explicit enough to check whether targeted literals were centralized or removed from runtime code
- [ ] The basics-first lane is recorded consistently in `DECISIONS.md`, `RECOVERY-CHECKLIST.md`, and `RECOVERY-ROADMAP.md`

### Platform Follow-On

- [ ] Re-enter the Supabase/Nhost contract lane only after the basics-first lock is recorded
...
- Missing-image lane is scoped without ambiguity.
- Hardcoding lane is scoped without ambiguity.
- Validation exists for both basics-first lanes.
