# Recovery Handover
- Timestamp: 2026-03-12T19:06:22
- Note: Ready for next chat with updated roadmap/checklist and current active frontier.
## Next Chat Prompt
```text
...
- Keep separating the 42-file `unused/` move wave from the older unrelated dirty state in `git status`, with `oando_website/*` tracked as a separate deletion cluster.
- Prioritize the catalog data-quality problems with critical image-path and missing-image failures in the workstation set before broad medium-severity metadata cleanup.
- Decide whether the next cleanup pass should target held duplicate families, remaining root tool-state/artifact folders, or the separate `oando_website/*` dirty-state investigation.
- Preserve the current duplicate-family holds and avoid any new source moves until the next cleanup target is chosen explicitly.
- Keep `/imprint`, `/news`, and `/support-ivr` on the uncertain-route list until a product/content decision reclassifies them.
- Generate a fresh recovery snapshot at the end of the work block.

## Latest Recovery State
# Recovery State

- Timestamp: 2026-03-12T19:06:12
- Branch: main
- Working tree entries: 219
- Modified: 35
- Added: 0
- Deleted: 149
- Renamed: 0
...
## Operator Note
Closed block after expanding recovery roadmap/checklist to current repo truth; archive-first marking policy remains active and next work starts from catalog/data remediation plus next-target selection.
