...
3. Record the source-label change from collapsed `nhost-backup` to explicit `nhost-graphql` and `nhost-sql`.

### Middle 24 Hours

1. Design and align the business-stats SQL rescue path.
2. Align the business-stats fallback chain to `Supabase -> Nhost GraphQL -> Nhost SQL -> stale cache -> safe default`.
3. Record whether any type/test/telemetry surfaces still assume the old single-source Nhost fallback label.

### Final 24 Hours

1. Define the mirror verification expansion for GraphQL and SQL.
2. Record the remaining legacy-table dependency check for runtime fallback.
3. Log any remaining non-blocking questions in `codex-recovery/OPEN-QUESTIONS.md`.
4. Generate a fresh recovery snapshot and leave the next follow-on step explicit.
...
- [ ] First 24 hours: parity baseline and fallback-source contract are recorded
- [ ] Middle 24 hours: business-stats SQL rescue design and fallback-source alignment are recorded
- [ ] Final 24 hours: mirror verification expansion and remaining legacy dependency review are recorded
...
- Keep `NEXT-PLAN.md` structured as a 72-hour block with task slices, not a short session-only plan.
...
- Timestamp: 2026-03-12T22:35:04
- Branch: main
- Working tree entries: 228
- Modified: 44
- Added: 0
- Deleted: 149
- Renamed: 0
...
## Operator Note
Converted NEXT-PLAN into a checklist-driven 72-hour block with first, middle, and final 24-hour task slices, and recorded that rule in recovery docs.
