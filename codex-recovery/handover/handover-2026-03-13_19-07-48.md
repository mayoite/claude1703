...
- Note: Homepage/products pass verified; follow-up remains for low-signal copy cleanup and build-log fallback noise capture
## Next Chat Prompt
```text
...
- Timestamp: 2026-03-13T07:54:55
- Branch: main
- Working tree entries: 274
- Modified: 82
- Added: 0
- Deleted: 145
- Renamed: 0
...
## Next 45 Minutes Plan
# Brand And Company Utility Polish / Next 72-Hour Block

Execution rule for this block: raise route quality on the brand-and-company utility surfaces without reopening the closed catalog-truth lane or fabricating unsupported product content.

## Objective

Turn the next frontend lane into a controlled brand-and-credibility pass by tightening route consistency across `/about`, `/sustainability`, `/career`, and `/social`, while keeping claims, proof, and document promises tied to current repo truth.
...
Completed items from the prior trust/legal/proof lane are recorded in `DECISIONS.md` and `RECOVERY-CHECKLIST.md` and are intentionally removed from this active execution checklist.
...
- The next bounded non-product utility cluster is `/about`, `/sustainability`, `/career`, and `/social`.
...
- The next frontend pass must stay inside repo-backed business identity, sustainability, contact, and proof language.
- The next frontend pass must not imply direct product downloads, unsupported certifications, or invented company proof.

## 72-Hour Sequence

### First 24 Hours

1. Start with `/about` and `/sustainability` as the first pair in the new bounded brand/company lane.
2. Confirm the route copy and current claims are supported by local typed data and existing public assets.
3. Keep the lane bounded to the four chosen utility routes only.

### Middle 24 Hours

1. Extend the same system to `/career` and `/social` only if the first pair verifies cleanly.
2. Keep CTA and contact flows aligned with the existing Resource Desk and contact model.
3. Re-run build verification after the route wave.

### Final 24 Hours

1. Record the brand/company utility wave outcome in recovery docs.
2. Leave one next explicit step only.
3. Keep the next frontier bounded instead of widening into product or configurator work.

## Detailed Checklist

### First 24 Hours

- [ ] Review `/about` and `/sustainability` plus their shared content/data sources for visual-system drift
- [ ] Confirm the route copy for this cluster already exists in `data/site/*` or extend it there first
- [ ] Confirm claims about company identity, sustainability, and contact paths are repo-backed
- [ ] Confirm shared `scheme-*`, button, and panel primitives can support the lane without adding a parallel styling system
- [ ] Record the chosen four-route cluster in recovery docs before broader edits

### Middle 24 Hours

- [ ] Upgrade `/about` and `/sustainability`
- [ ] Remove low-signal or placeholder-grade public copy from those routes
- [ ] Prefer typed route-copy updates over page-local literals where copy changes are needed
- [ ] Extend the same quality pass to `/career` and `/social` only if the first pair is verified
- [ ] Keep CTA and contact flows aligned with the current `Resource Desk` truth
- [ ] Run `npm run build`
- [ ] Capture any follow-up route gaps discovered during implementation

### Final 24 Hours

- [ ] Decide whether the full four-route utility cluster is complete or should stop after the first verified pair
- [ ] Update `NEXT-PLAN.md`, `DECISIONS.md`, and `RECOVERY-CHECKLIST.md`
- [ ] Generate a fresh recovery snapshot if the lane state materially changes
- [ ] Ensure `latest.md` reflects the newest snapshot
- [ ] Leave one next explicit step only
- [ ] Confirm the lane improved public brand/trust quality without violating catalog truth

## Completion Markers

- [ ] `/about` and `/sustainability` are materially improved and visually aligned to the newer system.
- [ ] `/career` and `/social` are materially improved if they stay in this bounded wave.
- [ ] Low-confidence or placeholder-grade copy is removed from the active public lane.
...
- Keep the lane bounded to `/about`, `/sustainability`, `/career`, and `/social` until verification closes it.

## Next Explicit Step

- Start the new bounded utility lane with `/about` and `/sustainability`, verify that pair, and only then decide whether to extend the same pass to `/career` and `/social`.
