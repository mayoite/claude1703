...
# Objective: Use the verified homepage layout recovery as the base for one final homepage-only visual QA pass on desktop and mobile, capturing any remaining rhythm, contrast, or footer-transition defects without reopening content truth or widening into multi-route redesign.
...
## Execution Sequence
### First Slice
1. Audit the rendered homepage on desktop and mobile after the verified layout/search pass.
2. Check the hero-to-footer rhythm, trust-strip calmness near the footer, and the final transition into the global footer.
3. Keep the lane bounded to the homepage experience only.

### Second Slice
1. Fix only any residual homepage-only spacing, contrast, or visual-density issues found in the desktop/mobile audit.
2. Preserve the new planner-led close, near-footer proof strip, and explicit desktop/mobile Enter-key search behavior.
3. Re-run verification after any residual polish.

### Final Slice
1. Decide whether the homepage can now close fully as a route block.
2. Record the visual QA outcome in recovery docs.
3. Leave one next explicit step only.

## Detailed Checklist
### First Slice

- [ ] Audit the rendered homepage on desktop
- [ ] Audit the rendered homepage on mobile
- [ ] Check the footer transition below the new planner-led close
- [ ] Check whether any homepage section still feels visually repetitive after the layout pass
- [ ] Record the visual QA cluster in recovery docs before any further edits

### Second Slice

- [ ] Fix only residual homepage spacing, contrast, or density issues if they appear in visual QA
- [ ] Preserve the new section order: hero -> AFC -> collections -> projects -> process -> trust -> close
- [ ] Preserve the single homepage closing conversion surface
- [ ] Preserve explicit Enter-key behavior on desktop and mobile search
- [ ] Run `npm run lint`
- [ ] Run `npm run build`
- [ ] Run `npx playwright test tests/homepage.spec.ts --workers=1`
- [ ] Capture any remaining follow-up gaps discovered during visual QA

### Final Slice

- [ ] Decide whether the homepage route block is now fully closed
...
- [ ] Confirm the lane improved homepage polish without violating repo truth
...
## Operator Note
Homepage layout/search recovery is implemented and verified. Next active frontier: one final homepage-only visual QA pass on desktop and mobile, focused on the trust-strip/footer transition and any residual spacing or density issues.
...
## Active Frontend Findings
- The homepage layout/search implementation pass is verified.
- The strongest current parts remain the hero, AFC banner, editorial project section, and the clearer planner-led close.
- The remaining uncertainty is visual, not structural: final desktop/mobile confirmation of section rhythm and the trust-strip-to-footer transition.
- The next pass should be a visual QA and residual-polish pass only.
...
- Timestamp: 2026-03-15T14:01:18

## Summary
Closed the bounded homepage truth-and-simplification pass after consolidating homepage truth into `data/site/homepage.ts`, normalizing homepage category links, reducing project cards to sector + company name truth, and simplifying the homepage closing contact model to planner + WhatsApp + phone. Fresh verification is green: `npm run lint`, `npm run build`, and `npx playwright test tests/homepage.spec.ts --workers=1`.
