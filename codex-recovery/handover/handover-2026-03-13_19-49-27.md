...
- Note: Category discovery is closed and verified; next active frontier is product detail excellence on /products/[category]/[product].
## Next Chat Prompt
```text
...
# Objective: Turn the next frontend lane into a controlled product-detail pass on `/products/[category]/[product]` by improving decision hierarchy, media behavior, specification readability, and CTA/trust continuity without reopening catalog-truth or configurator scope.
...
- Evidence before movement: PDP improvements must stay grounded in live catalog data, local product modules, and current quote/compare behavior.
- Archive before delete: no destructive cleanup in this block.
- Decision support beats novelty: prioritize faster comprehension, clearer specs, better media behavior, and cleaner quote intent over decorative churn.
- Small reversible steps: keep PDP polish incremental with verification after each wave.
...
- Keep compare, quote, and Resource Desk continuity honest to the current live route system.
...
- The next bounded high-impact cluster is the product detail surface under `/products/[category]/[product]`.
- The homepage, top-level `/products`, and category listing routes now provide cleaner entry and browse paths, so the biggest adjacent gap is decision quality once users open a product.
- The next pass must improve media hierarchy, spec readability, CTA placement, trust signals, and return-to-results continuity without widening into configurator or 3D reintegration.
- The next pass must stay inside repo-backed product truth, existing quote/compare flows, and current Resource Desk language.

## Execution Sequence
### First Slice
1. Audit `/products/[category]/[product]` and `ProductViewer` for hierarchy drift, weak spec readability, media fallback roughness, and CTA/trust placement issues.
2. Confirm current PDP copy, spec labels, and trust surfaces are grounded in live data or clearly reusable local modules.
3. Keep the lane bounded to the product detail route only.

### Second Slice
1. Improve PDP information hierarchy and media/summary flow for faster comprehension.
2. Improve specification readability, CTA placement, and quote/compare/Resource Desk continuity.
3. Re-run verification after the route wave.

### Final Slice
1. Record the PDP wave outcome in recovery docs.
2. Leave one next explicit step only.
3. Keep the next frontier bounded instead of widening into configurator or 3D work.

## Detailed Checklist
### First Slice

- [ ] Review `/products/[category]/[product]`, `ProductViewer`, and adjacent quote/compare surfaces for hierarchy and decision-support drift
- [ ] Confirm current spec labels, media behavior, and trust badges are grounded in live catalog data or clearly reusable local modules
- [ ] Confirm PDP claims stay repo-backed and do not imply unsupported document or media completeness
- [ ] Confirm the shared route, panel, and CTA primitives can carry this lane without adding a parallel styling system
- [ ] Record the product-detail cluster in recovery docs before broader edits

### Second Slice

- [ ] Upgrade PDP hierarchy and media-summary flow for faster scanability
- [ ] Improve spec readability and practical buying signals
- [ ] Improve quote, compare, breadcrumb, and Resource Desk continuity where already supported
- [ ] Improve mobile readability and CTA placement without weakening desktop clarity
- [ ] Run `npm run build`
- [ ] Capture follow-up PDP gaps discovered during implementation

### Final Slice

- [ ] Decide whether the product-detail lane is complete or needs one additional bounded follow-up pass
...
- [ ] Confirm the lane improved decision quality without violating catalog truth

## Completion Markers

- [ ] Product pages are materially improved and visually aligned to the newer system.
- [ ] Spec, trust, and CTA behavior are clearer on mobile and desktop.
...
- Keep the PDP lane grounded in live data rather than invented merchandising copy.
...
- Keep the lane bounded to `/products/[category]/[product]` until verification closes it.

## Next Explicit Step

- Audit `/products/[category]/[product]` and `ProductViewer` for media hierarchy, specification readability, CTA/trust placement, and return-to-results continuity before code edits begin.
