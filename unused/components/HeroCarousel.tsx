...
export function HeroCarousel() {
  ...
  const activeSlide = HERO_CAROUSEL_SLIDES[selectedIndex] || HERO_CAROUSEL_SLIDES[0];
              ...
              <div className="mt-8 flex flex-wrap items-center gap-3">
                {activeSlide.ctas.map((cta) => (
                  <Link
                    ...
                    className={
                      cta.variant === "primary"
                        ? "hero-carousel__cta inline-flex items-center gap-2 text-neutral-900 text-sm font-semibold tracking-[0.08em] uppercase px-6 py-3 transition-colors"
            ...
            <span className="relative block h-1 w-16 overflow-hidden rounded-full bg-white/30">
              {i === selectedIndex ? (
                <motion.span
                  ...
                  className="hero-carousel__bar absolute inset-y-0 left-0"
                />
              ) : null}
            </span>
